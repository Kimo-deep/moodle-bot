...
def run_moodle_engine(user, pwd):
    ...
    try:
        ...
        prompt = f"""
        رتب هذا التقرير الأكاديمي. لا تغير التصنيفات واجعل مسافة بين المعلومة والاخرى مثلا لا تجعل التكاليف ملتصقة في بعضها البعض اجعل بينهم سطر فاصل لمعرفة ان هذا تكليف يختلف عن الذي قبله وطبق نفس الشيء على الاختبارات واللقاءات والمحاضرات واجعل المعلومات مختصرة وغير مكررة:
