...
def run_moodle_engine(user, pwd):
    ...
    try:
        ...
        prompt = f"""
أنت مساعد أكاديمي متخصص في تنظيم التقارير الجامعية.

المطلوب:
تنسيق البيانات التالية في تقرير أكاديمي احترافي ومنظم وسهل القراءة مع الحفاظ الكامل على التصنيفات الأصلية.

قواعد التنسيق:
1. استخدم عناوين واضحة لكل تصنيف.
2. اجعل اسم المادة بارزاً.
3. أظهر اسم المدرس بشكل مستقل.
4. أظهر معلومات الفتح والإغلاق أو الموعد إن وجدت.
5. ميّز الواجبات والاختبارات والمحاضرات بشكل واضح.
6. افصل بين المواد المتشابهة إذا اختلف المدرس أو الشعبة.
7. لا تدمج المواد إلا إذا كانت البيانات متطابقة.
8. احذف التكرار غير الضروري.
9. اجعل التقرير مختصراً لكن شاملاً للمعلومات المهمة.
10. لا تعرض ملاحظات التسليم أو التعليمات بشكل منفصل.
11. إذا وُجدت ملاحظات تسليم أو تعليمات، ضعها مباشرة تحت الواجب أو الاختبار المرتبط بها فقط.
12. لا تُنشئ أقسام إضافية غير التصنيفات الموجودة.
13. إذا لم تتوفر معلومة معينة لا تذكرها.
14. لا تكتب أي شرح إضافي خارج التقرير.
15. استخدم تنسيقاً مرتباً مع مسافات وعناصر نقطية واضحة.

البيانات الأكاديمية:

📚 المحاضرات:
{lectures if lectures else 'لا يوجد'}

🎥 اللقاءات:
{meetings if meetings else 'لا يوجد'}

📝 الاختبارات:
{exams if exams else 'لا يوجد'}

⚠️ التكاليف والتجارب:
{assignments if assignments else 'لا يوجد'}
"""
