...
def run_moodle_engine(user, pwd):
    ...
    try:
        ...
        prompt = f"""
        رتب هذا التقرير الأكاديمي. لا تغير التصنيفات واجعل مسافة بين المعلومة والاخرى مثلا لا تجعل التكاليف ملتصقة في بعضها البعض اجعل بينهم مسافة لمعرفة ان هذا تكليف يختلف عن الذي قبله وطبق نفس الشيء على الاختبارات واللقاءات والمحاضرات:
